...
- 먼저 읽기: decks/my_first_presentation/docs/slide-map.md, guide/SLIDE_HTML_RULES.md
- 참고: template/slides/page_01.html
- 작성: decks/my_first_presentation/slides/page_NN.html
- slide-map 순서대로 한 행씩 처리해줘
- 각 행에서는 content_md에 적힌 장별 원고 하나만 읽고 화면용 슬라이드로 옮겨줘

화면에는 키워드만. 줄글 금지.
page_02를 만들 때만 decks/my_first_presentation/docs/02-toc.md도 읽고 목록을 슬라이드에 나열해줘 (목차 1장).
```

길어지면 위 요청을 `slide-map.md`의 page 단위로 나눈다. 이미 만든 슬라이드를 다시 읽힐 필요는 없다.
